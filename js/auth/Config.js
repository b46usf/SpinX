 ...
 * 
 * IMPORTANT: Set environment variables in Vercel Dashboard with prefix VITE_
 * Example: VITE_GOOGLE_CLIENT_ID, VITE_GAS_URL, etc.
 * 
...
function getEnvVar(name, required = false) {
  ...
  if (!value && required) {
    console.warn(`⚠️ Warning: ${name} is not set. Please configure in Vercel dashboard (VITE_${name})`);
  }
...
if (!isConfigured) {
  ...
  console.error('Please set VITE_GOOGLE_CLIENT_ID and VITE_GAS_URL in Vercel dashboard');
}
